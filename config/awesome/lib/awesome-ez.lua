...
function ez.key(keydef, keydata)
    local cb = keydata[1]
    local data = keydata[2]
    ...
    return awful.key(modkeys, key, ez.util.cb_from_table(cb), data)
end
...
function ez.keytable(tbl)
    local res = {}
    for keydef, keydata in pairs(tbl) do
        table.insert(res, ez.key(keydef, keydata))
    end
    return res
end
